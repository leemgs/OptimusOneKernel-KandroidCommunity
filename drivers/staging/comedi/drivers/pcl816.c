			    ...
			    struct comedi_subdevice *s)
{
	...
	if (devpriv->irq_blocked > 0) {
		...
		case INT_TYPE_AI1_INT:
		case INT_TYPE_AI3_INT:
			outb(inb(dev->iobase + PCL816_CONTROL) & 0x73, dev->iobase + PCL816_CONTROL);	
			udelay(1);
			outb(0, dev->iobase + PCL816_CONTROL);	
			     ...
			     int chanlen)
{
	...
	outb(devpriv->ai_act_chanlist[0] | (devpriv->ai_act_chanlist[seglen - 1] << 4), dev->iobase + PCL816_MUX);	
