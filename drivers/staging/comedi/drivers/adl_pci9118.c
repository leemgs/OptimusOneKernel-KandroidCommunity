...
struct pci9118_private {
	...
	void (*int_ai_func) (struct comedi_device *, struct comedi_subdevice *, unsigned short, unsigned int, unsigned short);	
			      ...
			      int backadd, int usedma, char useeos)
{
	...
	if (useeos) {
		...
		devpriv->chanlist[(2 * n_chan) ^ usedma] = devpriv->chanlist[0 ^ usedma];	
		useeos = 2;
	} else {
		useeos = 1;
	}
