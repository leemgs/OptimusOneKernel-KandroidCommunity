...
static void init_ctl_reg(struct gspca_dev *gspca_dev)
{
	...
	default:
		...
		reg_w_riv(dev, 0, 0x21ad, 0x00);	
		reg_w_riv(dev, 0, 0x21ac, 0x01);	
		reg_w_riv(dev, 0, 0x21a3, 0x00);	
		break;
	case BRIDGE_SPCA536:
...
static int sd_init(struct gspca_dev *gspca_dev)
{
	...
	case BRIDGE_SPCA504C:	
		...
		reg_w_riv(dev, 0xe0, 0x0000, 0x0001);	
	...
	default:
		...
		if (sd->subtype == AiptekMiniPenCam13) {
			...
			reg_w_riv(dev, 0x00, 0x270c, 0x05); 
...
static int sd_start(struct gspca_dev *gspca_dev)
{
	...
	case BRIDGE_SPCA504C:
		...
		reg_w_riv(dev, 0x0c, 0x0000, enable);	
		reg_w_riv(dev, 0xb0, 0x0000, enable);	

		
		reg_w_riv(dev, 0x30, 0x0001, 800);	
