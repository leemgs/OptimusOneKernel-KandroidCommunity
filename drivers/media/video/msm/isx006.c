...
#define ISX006_INTERVAL_T2		8	
#define ISX006_INTERVAL_T3		1	
#define ISX006_INTERVAL_T4		2	
#define ISX006_INTERVAL_T5		25	


#define ISX006_TOTAL_STEPS_NEAR_TO_FAR	30


#define REG_ISX006_INTSTS_ID			0x00F8	
#define REG_ISX006_INTCLR_ID			0x00FC	

#define ISX006_OM_CHANGED				0x0001	
#define ISX006_CM_CHANGED				0x0002	
...
static int32_t isx006_i2c_write(unsigned short saddr, unsigned short waddr, unsigned int wdata, enum isx006_width width)
{
	int32_t rc = -EIO;
  int int_buf[2];
	unsigned char *pbuf = ((unsigned char*)int_buf) + 2;  
...
static int isx006_reg_preview(void)
{
  ...
  rc = isx006_i2c_write(isx006_client->addr, 0x0032, 0x0100, WORD_LEN);  
  if (rc < 0)
    return rc;
  rc = isx006_i2c_write(isx006_client->addr, 0x0034, 0x0000, WORD_LEN);  
  if (rc < 0)
    return rc;
  rc = isx006_i2c_write(isx006_client->addr, 0x0036, 0x0000, WORD_LEN);  
  if (rc < 0)
    return rc;
...
static int isx006_snapshot_config(int width, int height)
{
  ...
  rc = isx006_i2c_write(isx006_client->addr, 0x0224, 0x00, BYTE_LEN);  
  if (rc < 0)
    return rc;

  rc = isx006_i2c_write(isx006_client->addr, 0x0014, 0x00, BYTE_LEN);  
  if (rc < 0)
    return rc;

  rc = isx006_i2c_write(isx006_client->addr, 0x001D, 0x00, BYTE_LEN);  
  if (rc < 0)
    return rc;
  
#if 1
  
  rc = isx006_i2c_write(isx006_client->addr, 0x0032, zoom_table[prev_zoom_step] + 0x0005, WORD_LEN);  
  if (rc < 0)
    return rc;
  rc = isx006_i2c_write(isx006_client->addr, 0x0034, zoom_offset_x, WORD_LEN);  
  if (rc < 0)
    return rc;
  rc = isx006_i2c_write(isx006_client->addr, 0x0036, zoom_offset_y, WORD_LEN);  
  if (rc < 0)
    return rc;
#endif
  
  rc = isx006_i2c_write(isx006_client->addr, 0x0011, 0x02, BYTE_LEN); 
  if (rc < 0)
    return rc;  
...
static int isx006_raw_snapshot_config(int width, int height)
{
  ...
  if (rc < 0)
...
#if 0 
	rc = isx006_i2c_write(isx006_client->addr, 0x02D3, 0x01, BYTE_LEN);  
	if (rc < 0)
		return rc;
#endif

	rc = isx006_i2c_write(isx006_client->addr, 0x0224, 0x01, BYTE_LEN);  
	if (rc < 0)
		return rc;
	...
	rc = isx006_i2c_write(isx006_client->addr, 0x001D, 0x1B, BYTE_LEN);  
	if (rc < 0)
		return rc;
  ...
  rc = isx006_i2c_write(isx006_client->addr, 0x00FC, 0x04, BYTE_LEN); 
  if (rc < 0)
    return rc;  
  ...
  do
  {
    ...
    isx006_i2c_write(isx006_client->addr, 0x00FC, 0x04, BYTE_LEN); 
...
static int isx006_check_focus(int *lock)
{
  ...
  rc = isx006_i2c_write(isx006_client->addr, 0x4014, 0x0F07, WORD_LEN); 
  if (rc < 0)
    return rc;
  rc = isx006_i2c_write(isx006_client->addr, 0x0103, 0x01, BYTE_LEN);  
  if (rc < 0)
    return rc;
  rc = isx006_i2c_write(isx006_client->addr, 0x0108, 0x01, BYTE_LEN);  
  if (rc < 0)
    return rc;
  rc = isx006_i2c_write(isx006_client->addr, 0x010D, 0x01, BYTE_LEN);  
  if (rc < 0)
    return rc;
  rc = isx006_i2c_write(isx006_client->addr, 0x0112, 0x01, BYTE_LEN);  
  if (rc < 0)
    return rc;
  rc = isx006_i2c_write(isx006_client->addr, 0x0117, 0x01, BYTE_LEN);  
  if (rc < 0)
    return rc;
  rc = isx006_i2c_write(isx006_client->addr, 0x011C, 0x01, BYTE_LEN);  
  if (rc < 0)
    return rc;
...
static int isx006_set_af_start(int mode)
{
  ...
  if (mode != FOCUS_MANUAL)
  {
    ...
    if (rc < 0)
      return rc;
    rc = isx006_i2c_write(isx006_client->addr, 0x0103, 0x00, BYTE_LEN);  
    if (rc < 0)
      return rc;
    rc = isx006_i2c_write(isx006_client->addr, 0x0108, 0x00, BYTE_LEN);  
    if (rc < 0)
      return rc;
    rc = isx006_i2c_write(isx006_client->addr, 0x010D, 0x00, BYTE_LEN);  
    if (rc < 0)
      return rc;
    rc = isx006_i2c_write(isx006_client->addr, 0x0112, 0x00, BYTE_LEN);  
    if (rc < 0)
      return rc;
    rc = isx006_i2c_write(isx006_client->addr, 0x0117, 0x00, BYTE_LEN);  
    if (rc < 0)
      return rc;
    rc = isx006_i2c_write(isx006_client->addr, 0x011C, 0x00, BYTE_LEN);  
    if (rc < 0)
      return rc;
  }
...
static int isx006_set_jpeg_rotation(int32_t rotation)
{
  ...
  rc = isx006_i2c_write(isx006_client->addr, 0x0202, jpeg_dri, WORD_LEN);  
...
static int32_t isx006_set_preview_mode(int32_t mode)
{
  ...
  { 
	...
	switch (mode) {
      ...
      isx006_i2c_write(isx006_client->addr,0x4016,0x243D,2);
      isx006_i2c_write(isx006_client->addr,0x4018,0x03CA,2);
      isx006_i2c_write(isx006_client->addr,0x401A,0x029B,2);
      isx006_i2c_write(isx006_client->addr,0x401C,0x0364,2);
      isx006_i2c_write(isx006_client->addr,0x401E,0x03BB,2);
      isx006_i2c_write(isx006_client->addr,0x4020,0x03CA,2);
      isx006_i2c_write(isx006_client->addr,0x02A4,0x0000,2); 
      isx006_i2c_write(isx006_client->addr,0x0383,0x02,1); 
      isx006_i2c_write(isx006_client->addr,0x0012,0x01,1); 
			break;

		case CAMERA_CAMCORD_MODE_FPS:
      isx006_i2c_write(isx006_client->addr,0x4016,0x1D03,2);	
      isx006_i2c_write(isx006_client->addr,0x4018,0x03FD,2);	
      isx006_i2c_write(isx006_client->addr,0x401A,0x0549,2);	
      isx006_i2c_write(isx006_client->addr,0x401C,0x0331,2);	
      isx006_i2c_write(isx006_client->addr,0x401E,0x02FE,2);	
      isx006_i2c_write(isx006_client->addr,0x4020,0x03CA,2);	
      isx006_i2c_write(isx006_client->addr,0x02A4,0x00C0,2);	
      isx006_i2c_write(isx006_client->addr,0x0383,0x02,1);	
      isx006_i2c_write(isx006_client->addr,0x0012,0x01,1);	
			break;
      ...
      isx006_i2c_write(isx006_client->addr,0x4016,0x1D03,2); 
      isx006_i2c_write(isx006_client->addr,0x4018,0x03FD,2); 
      isx006_i2c_write(isx006_client->addr,0x401A,0x0657,2); 
      isx006_i2c_write(isx006_client->addr,0x401C,0x0331,2); 
      isx006_i2c_write(isx006_client->addr,0x401E,0x048C,2); 
      isx006_i2c_write(isx006_client->addr,0x4020,0x03CA,2); 
      isx006_i2c_write(isx006_client->addr,0x02A4,0x0000,2); 
      isx006_i2c_write(isx006_client->addr,0x0383,0x03,1); 
      isx006_i2c_write(isx006_client->addr,0x0012,0x01,1); 
			break;
...
static int32_t isx006_set_zoom(int32_t mode)
{
  ...
  rc = isx006_i2c_write(isx006_client->addr, 0x0032, zoom_table[prev_zoom_step], WORD_LEN);  
  if (rc < 0)
    return rc;
  rc = isx006_i2c_write(isx006_client->addr, 0x0034, zoom_offset_x, WORD_LEN);  
  if (rc < 0)
...
static int isx006_init_sensor(const struct msm_camera_sensor_info *data)
{
  ...
  rc = isx006_i2c_write(isx006_client->addr, 0x0009, 0x1F, BYTE_LEN); 
  if (rc < 0)
    return rc;
...
int isx006_sensor_config(void __user *argp)
{
    ...
    case CFG_SET_JPEG_ROTATION:
      ...
      break;
		...
		case CFG_SET_ZOOM:
			rc = isx006_set_zoom(cfg_data.mode);  
			break;
