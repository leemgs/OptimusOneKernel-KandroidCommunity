...
#define ISX005_INTERVAL_T2		8	
#define ISX005_INTERVAL_T3		1	
#define ISX005_INTERVAL_T4		2	
#define ISX005_INTERVAL_T5		25	
...
#define REG_ISX005_INTSTS_ID			0x00F8	
#define REG_ISX005_INTCLR_ID			0x00FC	

#define ISX005_OM_CHANGED				0x0001	
#define ISX005_CM_CHANGED				0x0002	
...
static int isx005_reg_preview(void)
{
	...
	{
		rc = isx005_i2c_write(isx005_client->addr,0x4020,0x3202, WORD_LEN);       
		if (rc < 0)
			return rc;		
	}
	else if(prev_scene_mode != CAMERA_SCENE_NIGHT)
	{
		rc = isx005_i2c_write(isx005_client->addr,0x4020,0xFF01, WORD_LEN);    
		if (rc < 0)
			return rc;		
	}
  ...
  for(i=0; i<300;i++)
  {
      ...
      if(cm_changed_status & 0x0002)
      {
		...
		{
			isx005_i2c_write(isx005_client->addr,0x4018,0xCA03, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x401C,0xCA03, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x4020,0xFF01, WORD_LEN);
		}
		else if(iso_check_sts == CAMERA_ISO_100)
		{
			
			isx005_i2c_write(isx005_client->addr,0x4018,0x3301, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x401C,0x0100, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x4020,0x0100, WORD_LEN);
		}
		else if(iso_check_sts == CAMERA_ISO_200)
		{
			
			isx005_i2c_write(isx005_client->addr,0x4018,0x3004, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x401C,0x0100, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x4020,0x0100, WORD_LEN);
		}
		else if(iso_check_sts == CAMERA_ISO_400)
		{
			...
			isx005_i2c_write(isx005_client->addr,0x4018,0x6008, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x401C,0x0100, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x4020,0x0100, WORD_LEN);
		}
		else{ 
...
static int isx005_reg_snapshot(void)
{
	...
	if(prev_scene_mode == CAMERA_SCENE_SPORTS )
	{
		rc = isx005_i2c_write(isx005_client->addr,0x4020,0x9406, WORD_LEN);       
		if (rc < 0)
			return rc;		
	}
	else if(prev_scene_mode != CAMERA_SCENE_NIGHT)
	{
		rc = isx005_i2c_write(isx005_client->addr,0x4020,0xCA03, WORD_LEN);       
		if (rc < 0)
			return rc;		
	}
	...
	for(i=0; i<300;i++)
	{
		...
		if(iso_check_sts == CAMERA_ISO_AUTO)
		{
			isx005_i2c_write(isx005_client->addr,0x4018,0xCA03, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x401C,0xCA03, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x4020,0xCA03, WORD_LEN);
		}
		else if(iso_check_sts == CAMERA_ISO_100)
		{
			
			isx005_i2c_write(isx005_client->addr,0x4018,0x2f05, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x401C,0x0100, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x4020,0x0100, WORD_LEN);
		}
		else if(iso_check_sts == CAMERA_ISO_200)
		...
		else if(iso_check_sts == CAMERA_ISO_400)
		{
			...
			isx005_i2c_write(isx005_client->addr,0x4018,0xf60b, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x401C,0x0100, WORD_LEN);
			isx005_i2c_write(isx005_client->addr,0x4020,0x0100, WORD_LEN);
		}
		else{ 
...
static int isx005_set_sensor_mode(int mode)
{
    ...
    {
      if (prevew_mode_status == CAMERA_MODE_DEFAULT)  
        { 
...
static int isx005_set_af_start(int mode)
{
   ...
   printk(KERN_ERR "[#############################]\n");   
	...
	if(prev_af_mode == mode) {
             ...
             if(cm_light_check_sts_local >= 0x0994){ 
