...
sisfb_setupvbblankflags(struct sis_video_info *ivideo, u32 *vcount, u32 *hcount)
{
	...
	if((ivideo->currentvbflags & VB_DISPTYPE_DISP2) && (!(sisfb_bridgeisslave(ivideo)))) {
		...
		inSISIDXREG(SISPART1,(idx+0),reg1); 
		inSISIDXREG(SISPART1,(idx+1),reg2); 
		inSISIDXREG(SISPART1,(idx+2),reg3); 
		inSISIDXREG(SISPART1,(idx+3),reg4); 
...
sisfb_post_sis300(struct pci_dev *pdev)
{
	...
	orSISIDXREG(SISPART1, 0x24, 0x01);   
	...
	andSISIDXREG(SISPART1, 0x24, 0xfe);  
...
sisfb_post_xgi(struct pci_dev *pdev)
{
	...
	if(ivideo->revision_id == 0) {	
		...
		if(reg & 0x20) {
			setSISIDXREG(SISCR, 0x58, 0xd7, (reg & 0x10) ? 0x08 : 0x20); 
		}
	}
