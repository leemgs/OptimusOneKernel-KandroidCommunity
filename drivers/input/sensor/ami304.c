...
static int AMI304_Report_Value(int en_dis)
{
	...
	{
		input_report_abs(data->input_dev, ABS_RX, ami304mid_data.yaw);	
		input_report_abs(data->input_dev, ABS_RY, ami304mid_data.pitch);
		input_report_abs(data->input_dev, ABS_RZ, ami304mid_data.roll);
		input_report_abs(data->input_dev, ABS_RUDDER, ami304mid_data.mag_status);
		report_enable = 1;
	}

	if(atomic_read(&a_status))
	{
		input_report_abs(data->input_dev, ABS_X, ami304mid_data.nax);
		input_report_abs(data->input_dev, ABS_Y, ami304mid_data.nay);
		input_report_abs(data->input_dev, ABS_Z, ami304mid_data.naz);
		report_enable = 1;
	}

	if(atomic_read(&m_status))
	{
		input_report_abs(data->input_dev, ABS_HAT0X, ami304mid_data.nmx); 
		input_report_abs(data->input_dev, ABS_HAT0Y, ami304mid_data.nmy); 
		input_report_abs(data->input_dev, ABS_BRAKE, ami304mid_data.nmz); 
		input_report_abs(data->input_dev, ABS_WHEEL, ami304mid_data.mag_status);
		report_enable = 1;
	}
...
static int __init ami304_init(void)
{
	...
	ami304mid_data.controldata[0] = 200*1000; 
	ami304mid_data.controldata[1] = 0; 
	ami304mid_data.controldata[2] = 0; 
	ami304mid_data.controldata[3] = 1; 
	ami304mid_data.controldata[4] = 350; 
	ami304mid_data.controldata[5] = 10; 
	ami304mid_data.controldata[6] = 10; 
	atomic_set(&dev_open_count, 0);
