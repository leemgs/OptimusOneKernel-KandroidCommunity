...
uint sd_f2_blocksize = 64;		

#ifdef BCMSDYIELD
bool sd_yieldcpu = TRUE;		
uint sd_minyield = 0;			
bool sd_forcerb = FALSE;		
#endif
...
uint sd_power = 1;		
uint sd_clock = 1;		
uint8 sd_dma_mode = DMA_MODE_SDMA; 
uint sd_pci_slot = 0xFFFFffff; 
               ...
               void *params, int plen, void *arg, int len, bool set)
{
	...
	case IOV_SVAL(IOV_POWER):
		...
		if (sd_power == 1) {
			...
			sdstd_wreg8(si, SD_PwrCntrl, pwr); 
...
sdstd_host_init(sdioh_info_t *sd)
{
	...
	sd->caps = sdstd_rreg(sd, SD_Capabilities);	
...
sdstd_client_init(sdioh_info_t *sd)
{
	...
	if (sd->num_funcs >= 2) {
		set_client_block_size(sd, 2, sd_f2_blocksize );
...
sdstd_start_clock(sdioh_info_t *sd, uint16 new_sd_divisor)
{
	...
	sdstd_or_reg16(sd, SD_ClockCntrl, 0x1); 
...
sdstd_start_power(sdioh_info_t *sd)
{
	...
	sdstd_wreg8(sd, SD_PwrCntrl, pwr); 
...
void
sdstd_cmd_getrsp(sdioh_info_t *sd, uint32 *rsp_buffer, int count )
{
	int rsp_count;
