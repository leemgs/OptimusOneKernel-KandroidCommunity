...
void mtc_send_key_log_data(struct ats_mtc_key_log_type* p_ats_mtc_key_log)
{
  ...
  if(p_ats_mtc_key_log->log_id == 1) 
  {
    ...
    pRsp->mtc_rsp.log_data.log_type.log_data_key.hold = (unsigned char)((p_ats_mtc_key_log->x_hold)&0xFF);

    pRsp->mtc_rsp.log_data.log_type.log_data_key.keycode = Handset_7k_KeycodeTrans((p_ats_mtc_key_log->y_code)&0xFF);
  ...
  else 
  {
    ...
    pRsp->mtc_rsp.log_data.log_type.log_data_touch.screen_id = (unsigned char)1; 
...
{ 
	...
	{ MTC_LOG_REQ_CMD					,mtc_null_rsp					, MTC_ARM11_PROCESSOR}, 
#endif 
