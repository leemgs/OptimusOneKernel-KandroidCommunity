...
static void msm_fb_mddi_power_save(int on)
{
	...
	if (on) {
		...
		{
			MSM_FB_LCDC_VREG_OP(msm_fb_vreg_1[0], enable, 1800);
			MSM_FB_LCDC_VREG_OP(msm_fb_vreg_1[1], enable, 2800);
		}
		else
		{
		...
		if(lge_bd_rev >= LGE_REV_B)
		{
			MSM_FB_LCDC_VREG_OP(msm_fb_vreg_1[0], disable, 0);
			MSM_FB_LCDC_VREG_OP(msm_fb_vreg_1[1], disable, 0);
		}
		else
		{
...
struct device* lu3100_backlight_dev(void)
{
	return &bl_i2c_device.dev;
}

void __init lu3100_init_i2c_backlight(int bus_num)
{
...
void __init lge_add_lcd_devices(void)
{
	...
	lge_add_gpio_i2c_device(lu3100_init_i2c_backlight);
}
