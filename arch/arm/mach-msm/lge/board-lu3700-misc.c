...
#define PWM_MAX_HALF_DUTY		((GPMN_N_DEFAULT >> 1) - 60) 
...
static struct android_vibrator_platform_data lu3700_vibrator_data = {
	...
	.pwn_set = lu3700_vibrator_pwn_set,
...
static int lu3700_gpio_earsense_work_func(void)
{
	int state;
	int gpio_value;
...
static char *lu3700_gpio_earsense_print_state(int state)
{
	return ear_state_string[state];
}

static int lu3700_gpio_earsense_sysfs_store(const char *buf, size_t size)
{
	int state;
...
static unsigned lu3700_earsense_gpios[] = {
	GPIO_EAR_SENSE,
};

static struct lge_gpio_switch_platform_data lu3700_earsense_data = {
	.name = "h2w",
	.gpios = lu3700_earsense_gpios,
	.num_gpios = ARRAY_SIZE(lu3700_earsense_gpios),
	.irqflags = IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
	.wakeup_flag = 1,
	.work_func = lu3700_gpio_earsense_work_func,
	.print_state = lu3700_gpio_earsense_print_state,
	.sysfs_store = lu3700_gpio_earsense_sysfs_store,
};

static struct platform_device lu3700_earsense_device = {
	.name   = "lge-switch-gpio",
	.id = 1,
	.dev = {
		.platform_data = &lu3700_earsense_data,
	},
};
