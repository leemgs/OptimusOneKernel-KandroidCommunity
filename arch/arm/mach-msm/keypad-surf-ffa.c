...
int keypad_exist_scancode(unsigned char scancode)
{
	int index = 0, max_size = 0;

	max_size = ARRAY_SIZE(keypad_col_gpios) * ARRAY_SIZE(keypad_row_gpios);

	for (index = 0 ; index < max_size ; index++) {
		if (scancode == keypad_keymap_surf[index])
			return 1;
	}
	
	return 0;
}
EXPORT_SYMBOL(keypad_exist_scancode);
